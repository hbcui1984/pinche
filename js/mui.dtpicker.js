...
(function($, document) {
    ...
    var DtPicker = $.DtPicker = $.Class.extend({
        ...
        _renderDayText: function(val, date) {
            if (typeof this.options.renderDay === 'function') {
                return this.options.renderDay(val, date);
            }
            return val + (this.options.week ? ('（' + WEEKS[date.getDay()] + '）') : '');
        },
        _createDay: function(current) {
            ...
            if (options.customData.d) {
                ...
                for (; d <= maxDay; d++) {
                    ...
                    dArray.push({
                        text: self._renderDayText(val, new Date(parseInt(this.ui.y.picker.getSelectedValue()), parseInt(this.ui.m.picker.getSelectedValue()) - 1, d)),
                        value: val
                    });
                }
            }
